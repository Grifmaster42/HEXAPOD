...
from DRIVE_RED.dynamixel_EL import *
...
class ServoAx12a(Dynamixel):
    ...
    def __init__(self, id):
        super().__init__(id)
        self.setReturnLevel(self.RETURN_LEVEL_READ_COMMANDS)
    ...
    def getReturnDelay(self):
        delay = list(self._requestNByte(self.__RETURN_DELAY_TIME,1))
        return delay
    ...
    def getReturnLevel(self):
        lvl = list(self._requestNByte(self.__RETURN_LEVEL,1))
        return lvl
    ...
    def getGoalPosition(self):
        i = list(self._requestNWord(self.__GOAL_POSITION, 1))
        goal = i[0] + (i[1] << 8)
        return goal
    ...
    def getMovingSpeed(self):
        i = list(self._requestNWord(self.__MOVING_SPEED, 1))
        movSpe = i[0] + (i[1] << 8)
        return movSpe
    ...
    def getPresentPosition(self):
        i = list(self._requestNWord(self.__PRESENT_POSITION, 1))
        presPos = i[0] + (i[1]<<8)
        #print("aktuelle Pos: ",presPos, i)
        return presPos
    ...
    def getPresentSpeed(self):
        i = list(self._requestNWord(self.__PRESENT_SPEED, 1))
        presSpe = i[0] + (i[1] << 8)
        return presSpe
    ...
    def getGoalPosSpeed(self):
        l = list(self._requestNWord(self.__GOAL_POSITION, 2))
        i = l[0:2]
        goal = i[0] + (i[1] << 8)
        j = l[2:4]
        presSpe = j[0] + (j[1] << 8)
    ...
    def getPresPosSpeed(self):
        l = list(self._requestNWord(self.__PRESENT_POSITION, 2))
        i = l[0:2]
        presPos = i[0] + (i[1] << 8)
        j = l[2:4]
        presSpe = j[0] + (j[1] << 8)
        return presPos, presSpe
    ...
    # return    integer
    @staticmethod
    def __convertByteToInt(nByte: list):
